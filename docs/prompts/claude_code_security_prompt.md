# Claude Code Security Review Prompt (Hardened & Complete)

> **Source:** Leaked Claude Code automated PR security review prompt (`src/commands/security-review.ts`).  
> **Hardened Enhancements:**  
> 1. **Indirect Prompt Injection Defense:** Explicit `<untrusted_diff>` and `<untrusted_finding>` boundaries with strict passive-data directives.  
> 2. **Sub-Task Contamination Shield:** Prevents attacker payloads from multiplying across parallel review sub-tasks.  
> 3. **Structured Output Adaptability:** Dual-mode output format (Human Markdown & Machine JSON/Pydantic schema).
